import { Color3, DirectionalLight, Effect, HemisphericLight, Mesh, MeshBuilder, Scene, ShaderMaterial, ShadowGenerator, Vector3 } from '@babylonjs/core';
...
export class Atmosphere {
  ...
  readonly sky:Mesh; private shader:ShaderMaterial; time=16.5; auto=true;
  private lamps: Array<{light:import('@babylonjs/core').PointLight; position:Vector3}> = [];
  constructor(private scene:Scene, private mats:MaterialLibrary, graphics:GraphicsSettings) {
    ...
    this.sky.material=this.shader; this.sky.isPickable=false; this.sky.infiniteDistance=true;
  ...
  dispose(){ this.sky.dispose(); this.shader.dispose(); this.shadows?.dispose(); this.sun.dispose(); this.ambient.dispose(); }
}
