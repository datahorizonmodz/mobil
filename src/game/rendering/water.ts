import { Color3, Effect, Mesh, MeshBuilder, Scene, ShaderMaterial, StandardMaterial, Vector3 } from '@babylonjs/core';
...
export class Water {
  mesh:Mesh; material:ShaderMaterial|null=null; private fallback:StandardMaterial;private clock=0;
  constructor(scene:Scene,subdivisions=32,advanced=true) {
    this.fallback=new StandardMaterial('reliable-water',scene);
    this.fallback.diffuseColor=new Color3(.06,.32,.39);this.fallback.specularColor=new Color3(.38,.57,.60);
    this.fallback.alpha=.86;this.fallback.backFaceCulling=false;
    this.mesh=MeshBuilder.CreateGround('coastal-ocean',{width:4200,height:1700,subdivisions},scene);
    this.mesh.position.set(0,-.75,-1320);this.mesh.material=this.fallback;this.mesh.isPickable=false;this.mesh.renderingGroupId=1;
    if(advanced){
      ...
      this.material.backFaceCulling=false;this.mesh.material=this.material;
      this.material.forceCompilation(this.mesh,undefined,undefined,reason=>{
        console.error('[Coastline] Water shader failed; switching to reliable water:',reason);
        this.mesh.material=this.fallback;this.material?.dispose();this.material=null;
      });
    }
  }
  ...
  get shaderReady(){return !this.material||this.material.isReady(this.mesh);}
  dispose(){this.mesh.dispose();this.material?.dispose();this.fallback.dispose();}
}
